...
//import DiskStorage
//
//open class EtvnetApiAsyncClient: ApiClient {
...
//  }
//
//  public func authorization(includeClientSecret: Bool = true) async throws -> AuthResult? {
...
//    } else {
//      if let value = try await authClient.getActivationCodes(includeClientSecret: includeClientSecret) {
...
//          if let value = try await updateToken(refreshToken) {
//            ok = true
...
//            if let value = try await authClient.createToken(deviceCode: deviceCode) {
//              ok = true
//
//              configFile.items = value.asMap()
//              try saveConfig()
...
//  }
//
//  public func createToken(deviceCode: String, delayCallback: () -> Void = { sleep(5) }) async -> AuthProperties? {
...
//  }
//
//  func updateToken(_ refreshToken: String) async throws -> AuthProperties? {
//    try await authClient.updateToken(refreshToken: refreshToken)
//  }
//
...
//
//      let response = try await request(path, method: method, queryItems: newQueryItems, headers: headers)
//
...
//
//            if let refreshToken = refreshToken {
//              if let fullValue = try await updateToken(refreshToken) {
//                configFile.items = fullValue.asMap()
//                try saveConfig()
//
//                result = try await self.fullRequest(path: path, to: type, method: method, queryItems: queryItems, unauthorized: true)
//              }
//            }
//          }
//        }
